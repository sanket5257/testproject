...
const candidates = [
  { name: "Alice Johnson", date: "27-02", email: "alice.johnson@example.com", phone: "123-456-7890", resume: "one.pdf", status: "Selected" },
  { name: "Bob Smith", date: "02-02", email: "bob.smith@example.com", phone: "987-654-3210", resume: "one.pdf", status: "View" },
  { name: "Catherine Lee", date: "01-01", email: "catherine.lee@example.com", phone: "555-123-4567", resume: "one.pdf", status: "Rejected" },
  { name: "David Brown", date: "03-03", email: "david.brown@example.com", phone: "444-987-6543", resume: "one.pdf", status: "View" },
];
    ...
    <div className=" min-h-screen font-Manrope bg-white text-black ">
      ...
      <div className="w-2/10 h-full p-12 bg-white shadow-2xl flex flex-col justify-start gap-5 items-center">
        ...
        <div className="flex justify-center items-center">
                        <img className="size-5" src="../employerimg/flash.svg" alt="" />
        ...
        <div className="flex text-md justify-between border w-full  text-[#33CA0E] rounded-lg px-3 py-4 cursor-pointer ">
                    <div className="flex justify-center items-center">
                        <img className="size-5" src="../employerimg/whatsappicon.png" alt="" />
        ...
        {["All", "Pending", "Reviewed", "Rejected"].map(index => (
          <div key={index} className="flex text-sm justify-between border w-full  text-gray-700 rounded-lg px-3 py-3 cursor-pointer ">
       ...
       <input type="radio" name="Country"/>
        ...
        <button onClick={() => setSortOrder("Newest first")} className="block  p-2 border rounded-lg">Newest first</button>
        <button onClick={() => setSortOrder("Oldest first")} className="block  p-2 border rounded-lg">Oldest first</button>
        <button onClick={() => setSortOrder("Oldest first")} className="block  p-2 border rounded-lg">A-Z</button>
        <button onClick={() => setSortOrder("Oldest first")} className="block  p-2 border rounded-lg">Z-A</button>
          ...
          <tbody>
            {sortedCandidates.map((candidate, index) => (
              <tr key={index} className="text-start">
                ...
                <td className=" p-4 text-blue-500"><h1 className=" bg-[#F5F1FE] rounded-xl text-sm text-center">{candidate.resume}</h1></td>
                <td className=" p-4 ">
                  {candidate.status === "Selected" ? <span className="bg-[#34A853] text-white px-4 py-2 rounded">Selected</span> :
                   candidate.status === "Rejected" ? <span className="bg-gray-500 text-white px-4 py-2 rounded">Rejected</span> :
                   <a className="bg-[#004C99] text-white px-8 py-2  rounded">View</a>}
                </td>
              </tr>
            ))}
          </tbody>
