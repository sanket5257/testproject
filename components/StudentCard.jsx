...
const StudentCard = () => {
  ...
  return (
      ...
      >
        ...
        <div className=" md:h-[60vh] h-[30vh] w-full no-scrollbar overflow-x-auto flex justify-start pl-4 items-center  gap-4  rounded-xl">
          ...
          {student.map((student,index)=>{
            return (
                <div key={index} id={`student${index}`} className=" relative h-full flex shrink-0 justify-between items-end rounded-2xl overflow-hidden text-white font-Montserrat md:w-[26vw] ">
    ...
    <div id='student-info' className='flex opacity-0 hover:opacity-100 flex-col items-center  p-6 justify-center w-full h-1/2 absolute'>
      ...
      <h1 className='z-10 md:text-4xl font-Montserrat font-bold '>{student.salary}</h1>

      <p className='z-10 text-[1.2vw]'>" I am {student.position}. I was placed through <span className='font-bold'>{student.platform}</span> platform in <span className='font-bold'>{student.company}</span> with <span className='font-bold'>{student.salary}</span>"</p>
      
      <h1 className='z-10 ml-[15vw] text-[1.2vw] font-Montserrat font-bold '>~{student.name}</h1>
